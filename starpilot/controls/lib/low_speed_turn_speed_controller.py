...
class LowSpeedTurnSpeedController:
  def __init__(self, StarPilotVCruise):
    ...
    self._predictive_turn_entered = False
  ...
  def _close_predictive_turn(self):
    # Only learn if the turn was actually completed (steering crossed 30°).
    # If aborted before the wheel turned (red light, traffic), discard silently.
    if self._predictive_bucket_key is not None and self._predictive_turn_entered:
  ...
  def calibration_log(self, v_ego, sm, in_low_speed_turn):
    ...
    # Track predictive turn state during calibration so pre-turn budget also learns.
    # Open the bucket on blinker approach (before 30°), close when turn ends.
    blinker_active = self._last_blinker_dir != 0
    pre_turn_approach = (blinker_active
                         and not in_low_speed_turn
                         and LSTSC_MIN_SPEED < v_ego < LSTSC_PREDICTIVE_MAX_SPEED
                         and not sm["carState"].standstill
                         and not sm["carState"].steeringPressed
                         and bool(sm["starpilotCarState"].alwaysOnLateralEnabled))

    if pre_turn_approach and not self._predictive_turn_in_progress:
    ...
    # Track peak torque across both approach and reactive phases
    ...
    if not eligible:
      if was_active and self.torque_data:
        ...
        self.starpilot_planner.params.put_nonblocking("LowSpeedTurnCalibrationProgress", self.calibration_progress)
        self._persist_timer = 0.0
      # Turn just ended — close predictive tracking if it was open
      if was_active and self._predictive_turn_in_progress:
        self._close_predictive_turn()
      return

    self.calibrate_mode_active = True
    self.enable_training = True
    self._predictive_turn_entered = True  # steering ≥30°: turn is real
