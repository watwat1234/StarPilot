...
- `/code-review high` run against `302eaaed0^..b14d21e15` (the full diff: implementation + test fix + this status writeup). No blocking findings. Notable ones, filtered for relevance:
  - **Excusal durability risk**: `allow_empty_buckets` only excuses a bucket while its count is exactly `0`. If Bucket 0 ever picks up even a single real point (e.g. one sharp maneuver on some future drive), it stops being excused and needs its full per-bucket minimum (100 points) to satisfy `is_valid()` again — same requirement as before this fix. If that steering range then never gets revisited, the car reverts to permanently `liveValid=False`, just with 1-99 stray points sitting there instead of 0. Not unsafe (worst case is a return to today's status quo) and not something to pre-emptively fix, but worth watching for across many future drives — the fix's durability rests on Bucket 0 staying at literal zero indefinitely, which two drives' telemetry supports but doesn't guarantee forever.
    - **This happened.** Route 34→35 (2026-09-12) showed exactly this: `liveValid` dropped `True`→`False` (`calPerc` 100%→62%) between drives with `totalBucketPoints` unchanged. Follow-up fix planned in `.scratch/ioniq_torqued_bucket_excusal_fix_plan.md` — excuse Hard Left/Hard Right regardless of fill level, not only while exactly empty.
  - **Diagnostic tooling doesn't know about the flag**: `tools/tuning/inspect_torque_buckets.py:202` and `tools/tuning/inspect_lateral_tuning.py:369` construct `TorqueEstimator` without `allow_empty_buckets`, and each also has its own hand-rolled `calc_cal_perc`/`lowest_pct` computation (`inspect_torque_buckets.py:343`, `inspect_lateral_tuning.py:693`) that duplicates `get_valid_percent()` with no excusal logic at all. Replaying an Ioniq 6 route through either tool will print `is_calculable()/is_valid()=False`, a `calPerc` capped near 50%, and Bucket 0 flagged "BLOCKING" even when the car's real on-device `liveValid` is `True` and `calPerc` is 100%. Relevant to this plan's own step 4 below (cross-checking the live fit against offline SVD numbers) — read the tool's output knowing it will misreport calibration state on this car until patched to pass the flag through.
  - **Estimator's `is_calculable()` gate is looser than intended for the Raw fields**: with `allow_empty_buckets=True`, `is_calculable()` only requires 3 total points anywhere (not 3 in a real-data bucket specifically), so `estimate_params()` can run SVD on as few as 3 near-collinear points from a single narrow bucket while all 7 others are still empty-and-excused at boot. This can produce degenerate/garbage `latAccelFactorRaw`/`latAccelOffsetRaw` transiently. Confirmed this doesn't reach the actually-used `Filtered`/`liveValid` values or affect steering: `is_valid()` (the gate for `update_params()`) still independently requires every non-excused bucket to clear its own real per-bucket minimum (100-500 points) plus the total-points floor (4000), so the control-affecting path is unaffected. Diagnostic-only impact.
  - Lower-priority nits raised, no action taken: the `CAR` import/fingerprint comparison in `main()` runs unconditionally for every brand rather than only Hyundai (cheap, harmless today); `allow_empty_buckets` lives on the shared `PointBuckets` base class rather than something more scoped (already a deliberate, documented tradeoff in this plan's scope decision); the fingerprint flag is computed in `main()` rather than derived inside `TorqueEstimator.__init__` from `CP` directly (deriving it internally would also incidentally fix the tooling gap above, since the tuning scripts already construct `TorqueEstimator` with a real `CP`); the `>=3` floor in `is_calculable()` is a bare magic number in production code with its rationale documented only in a test comment and this plan doc, not inline.
